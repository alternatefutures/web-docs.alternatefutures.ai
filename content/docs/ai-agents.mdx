...
- Deploying a team of agents? [Deploy a swarm of agents](/guides/deploy-a-swarm) has
  the flag form of every wizard step (`acc create agent`, `acc create swarm`,
  `acc swarms deploy --yes`).
- Prefer a template when one fits: the [template catalog](/templates/catalog) lists
  every ID with its required variables, generated from the platform source.
- Calling the API directly? The [GraphQL API reference](/api) is generated from the
  server schema on every merge, so its queries and mutations match production.
