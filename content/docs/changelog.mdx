...
- **Web App** - Dashboard for managing projects, sites, agents, and billing at [clouds.alternatefutures.ai](https://clouds.alternatefutures.ai)
...
- Migration guides from other static hosting platforms
...
- **SSL Proxy** - Pingap-based SSL termination proxy on decentralized compute with a dedicated IP
- **Auth Service** - Hono + SQLite authentication service on decentralized compute
- **Secrets Management** - Infisical deployment for secure credential storage
- **Decentralized Container Registry** - Self-hosted registry with IPFS-backed storage
