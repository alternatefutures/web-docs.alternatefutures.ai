---
title: "Projects"
description: "Projects group your services, deployments, domains, and team members. Create and switch projects from the web app or the acc CLI."
---

Projects are the top-level organizational unit in Alternate Clouds. Every
service belongs to exactly one project, and the CLI always acts on your active
project.
...
- **Services** - Containers, templates, and GPU workloads you have deployed
- **Deployments** - The release history of each service
- **Domains** - Custom domains pointed at your services
- **Members** - The people who can see and manage the project
...
There are no fixed per-project resource quotas - what you can run is governed
by your subscription plan and your credit balance. Use per-service budgets
(`--budget-monthly`, `--budget-total`, `--stop-hours`) to cap spend; see
...
```bash
# Inspect the service you want to move
acc services info <service-id>
```

### Recreate in the Target Project
```bash
# Switch to target project
acc projects switch <target-project-id>

# Recreate the service (template, image, resources, env vars)
acc services create --kind docker --image <ref> --port <n>
```
...
<Callout type="warn" title="Danger">
...
- [Quick start](/guides/quickstart) - Deploy your first service
- [Access tokens](/guides/api-keys) - Tokens for CI and agents
- [Billing and usage](/guides/billing) - Credits, budgets, and plans
