...
description: "Deploy serverless edge functions on decentralized infrastructure with Alternate Futures Functions."
---

<Callout type="warn" title="Deploy functions from the web dashboard">
Cloud Functions are live in the web app: add a service, pick **Function**, and
edit the source right in the dashboard. The `af functions` CLI commands on this
page are retired, and `acc services create --kind function` is not wired up
yet - use the dashboard until CLI support ships.
</Callout>

Deploy serverless edge functions on decentralized infrastructure with Alternate Clouds Functions.
