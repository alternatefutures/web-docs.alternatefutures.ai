...
2. **Agents**: add each agent with a one-word name (that is how you will
   ...
   list. **Edit the full persona** under the sentence opens the whole text the
   agent follows, if one sentence is not enough.
...
- **What is its job?** One sentence. It becomes the first line of
  `personas/researcher.md`; edit that file any time, or the persona text under
  the agent on **Source › Agents** in the web app.
...
**Edit the team.** In the web app, change the agents on **Source** (name,
model, what it does or the full persona) and click **Save** in the footer:
that registers a new version. The banner then says the changes are not
deployed; click **Redeploy** and confirm **Stop and redeploy**. The model a
team runs is the one set on its agents; a deploy applies it. From the CLI,
edit the files, then stop and deploy:
