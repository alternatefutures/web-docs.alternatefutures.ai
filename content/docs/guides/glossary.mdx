...
Filecoin, Arweave, functions) are covered in [Retired af CLI guides](/legacy).
...
A decentralized marketplace of independent compute providers. General
container workloads run on Akash providers.
...
A self-hosted, OCI-compatible container registry that stores image layers on
IPFS and runs on Akash. See [Decentralized container registry](/guides/decentralized-registry).
...
The InterPlanetary File System, a content-addressed storage network. Used by
the decentralized registry to store image layers.
...
The Open Container Initiative, the standard for container images and
registries. Standard Docker tools work with anything OCI-compatible.
...
A network of providers offering confidential computing on TEE hardware.
Confidential services run there.
...
A network of GPU providers. GPU services run there.
